export async function detectHandoff(state) {
    const signal = await state.readHandoffSignal();
    if (!signal)
        return null;
    const normalized = signal.trim().toUpperCase();
    if (normalized.includes('TASK_FULLY_COMPLETE'))
        return 'complete';
    if (normalized.includes('HANDOFF_COMPLETE'))
        return 'handoff';
    return null;
}
export async function generateRescueHandoff(state, sessionNum, originalTask) {
    // Preserve existing knowledge
    const existingKnowledge = await state.readKnowledge();
    // Generate rescue progress
    await state.writeProgress(`## STATUS: INTERRUPTED\n\nSession ${sessionNum} was interrupted before writing handoff files.\nThis is an auto-generated rescue handoff.\n`);
    // Generate rescue prompt
    await state.writeNextPrompt(`# Rescue Handoff — Continuing from session ${sessionNum}\n\n` +
